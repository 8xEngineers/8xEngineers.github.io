...
- Although the plugin does not have any bugs, some parts of the source code seem like "band-aid" solutions to the problems. For example, we noticed this part of the code - 
    ...
    Moreover, there are some potential problem on coding practice. For example, we notices that in the `bigfoot-popover.scss` there are several instance of hardcoded value and magic numbers:
    ```
